...
export function BarberHistory() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [appts, brbs] = await Promise.all([
        apiClient.get<Appointment[]>("/api/appointments?status=COMPLETED"),
  ...
  const inputCls =
    "w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary [color-scheme:dark]"
      ...
      <Card className="mb-4">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Entrada">
              <input type="time" value={barber?.workStart ?? "09:00"} disabled className={inputCls} />
            </Field>
            <Field label="Salida">
              <input type="time" value={barber?.workEnd ?? "20:00"} disabled className={inputCls} />
            </Field>
          </div>
        </div>
      </Card>
              ...
              <div key={a.id} className="flex items-center justify-between py-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {a.service?.name ?? "—"} · {formatDate(a.date)} · {a.time}
                  </p>
                </div>
                <span className="font-medium tabular-nums text-primary">
                  {money(a.service?.price ?? 0)}
                </span>
              </div>
            ))}
          </div>
